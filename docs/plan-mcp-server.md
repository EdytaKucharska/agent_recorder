...
const STRIPPED_KEYS = new Set([
  "prompt", "system_prompt", "reasoning",
  "chain_of_thought", "messages", "thought", "thinking"
]);
...
export function buildEventTree(events: BaseEvent[], maxDepth: number): EventNode[]
...
export function listSessionsSummary(
  ...
  opts: { limit: number; offset: number; status?: SessionStatus; since?: string; upstreamKey?: string }
): SessionSummaryRow[]
```
...
export function queryTokenUsageAggregated(
  ...
  opts: {
    ...
    sessionId?: string; upstreamKey?: string;
    limit: number;
  }
): AggregatedTokenRow[]
```
...
```typescript
export function register(server: McpServer, db: Database.Database, opts: McpServerOptions): void
```
